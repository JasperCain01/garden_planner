...
| **GitHub Pages is enabled on the wrong source, and the app is not live.**  | Intentionally deferred as post-v1 backlog — the Settings half is a repo-admin action no session can perform. Pages currently builds from `main`, and **there is no `gh-pages` branch at all** (verified against the remote), so the order is forced: `npm run deploy` is what creates the branch, and only then can Settings → Pages select it. Unblocked by one person, once, in that order — deploy, switch the source, `npm run smoke:deployed`. See README.md's "Deployment" section and ADR 0024's dated note. |
