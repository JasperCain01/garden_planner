...
| **GitHub Pages is enabled on the wrong source, and the app is not live.**  | Intentionally deferred as post-v1 backlog — it is a repo-**Settings** action no session can perform. Pages currently builds from `main` rather than the `gh-pages` branch `npm run deploy` publishes to (see README.md's "Live site" and ADR 0024's dated note). Unblocked by one person, once: change the source, run `npm run deploy`, run `npm run smoke:deployed`.                                                                            |
