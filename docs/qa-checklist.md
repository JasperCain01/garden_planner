...
| -------------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Full E2E regression  | `PW_EXECUTABLE_PATH=/path/to/chromium npm run e2e`                                                      | 27/27 passing (UI redesign Phase 4 — was 7/7 through Stage 6.4, 13/13 after Phase 1's `workspace-layout.spec.ts`, 18/18 after Phase 2's `canvas-scale.spec.ts`, 22/22 after Phase 3's `palette.spec.ts`; `plot-settings.spec.ts` adds five, holding Phase 4's acceptance criteria). **Blocking in CI.** `plot-export.spec.ts`'s long-standing flake was diagnosed and fixed in Phase 1 — see §5. Note that `canvas-scale.spec.ts`'s two pixel-differencing specs take 33s and 39s against a 30s default timeout on a slow machine; they are not flaky, they are slow, and `--timeout=90000` is the honest way to run them there. (Phase 4's container ran them in 21.8s and 19.8s, i.e. inside the default — the note is for the slower machine, and 20s against a 30s budget is why it stays.) |
| Accessibility (axe)  | `npm run a11y -w app` (needs the preview server running)                                                | 0 violations, all eight scanned states (unchanged since Stage 6.2; each state arrived with the surface it scans — the add-crop modal in UI redesign Phase 1, edit-shape mode and the clear-all confirmation in Phase 2, an expanded palette card in Phase 3, the warnings dock with a warning in it and the soil disclosure open in Phase 4). **Blocking in CI.**                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Keyboard walkthrough | `PW_EXECUTABLE_PATH=/path/to/chromium npm run keyboard-walkthrough -w app`                              | All steps pass, 20 tab presses to the canvas (35 before UI redesign Phase 1 moved the add-crop form's ~25 stops behind a dialog, then 15, then 20 once Phase 2's canvas toolbar joined the path) — unchanged by Phases 3 and 4. Two skip links, and a step each for the dialog, zoom, edit-shape, Phase 3's card disclosure and Phase 4's warnings-dock "Show me"; see `docs/accessibility.md` §6–§9. **Informational in CI.**                                                                                                                                                                                                                                                                                                                                                                  |
